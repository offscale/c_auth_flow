...
/*
 * Authentication flow (including spinning up of local web server, and setting of project ID)
 * */
extern struct GoogleCloudProject get_google_auth(const char *client_id, const char *client_secret,
                                                 const char *refresh_token) {
    JSON_Object *tokens_obj_json;

    if (refresh_token == NULL) {
        const struct StrStr auth_flow_code_resp = auth_flow_user_approval(client_id);
        const char *redirect_uri = auth_flow_code_resp.first, *code = auth_flow_code_resp.second;

        const JSON_Value *tokens_json = auth_flow_get_tokens(client_id, client_secret, code, redirect_uri);
        tokens_obj_json = json_value_get_object(tokens_json);
        refresh_token = json_object_get_string(tokens_obj_json, "refresh_token");
    } else {
        const JSON_Value *tokens_json = auth_flow_get_tokens_from_refresh(client_id, client_secret, refresh_token);
        tokens_obj_json = json_value_get_object(tokens_json);
    }

    {
        const char *google_access_token = json_object_get_string(tokens_obj_json, "access_token");
        const double google_access_token_expires_in = json_object_get_number(tokens_obj_json, "expires_in");
        const time_t google_access_token_expiry = time(NULL) + (longest) google_access_token_expires_in;
        struct GoogleCloudProject project;

        const JSON_Object *project_object_json = get_project(google_access_token);
        if (!json_object_has_value(project_object_json, "projectId")) no_projects_error();


        project.projectNumber = json_object_get_string(project_object_json, "projectNumber");
        project.projectId = json_object_get_string(project_object_json, "projectId");
        project.lifecycleState = json_object_get_string(project_object_json, "lifecycleState");
...
inline struct StrStr auth_flow_user_approval(const char *client_id) {
    const char *redirect_uri = "http://" SERVER_HOST ":" PORT_TO_BIND_S EXPECTED_PATH;
...
inline JSON_Value *auth_flow_get_tokens(const char *client_id, const char *client_secret,
...
inline JSON_Value *auth_flow_get_tokens_from_refresh(const char *client_id,
...
inline const JSON_Object *get_project(const char *google_access_token) {
    ...
    rc = curl_url_set(urlp, CURLUPART_HOST, "cloudresourcemanager.googleapis.com", 0);
    ...
    if (response.status_code > 299 || response.body == NULL || response.code != CURLE_OK) {
...

    {
        ...
        const JSON_Array *projects_items_json = json_object_get_array(json_value_get_object(json_value), "projects");
        const size_t projects_items_n = json_array_get_count(projects_items_json);
        size_t i;

        if (projects_items_n == 0) no_projects_error();

        for (i = 0; i < projects_items_n; i++) {
            const JSON_Object *project_obj = json_array_get_object(projects_items_json, i);
            if (strcmp(json_object_get_string(project_obj, "lifecycleState"), "ACTIVE") == 0) {
                const char *projectId = json_object_get_string(project_obj, "projectId");
                const size_t projectId_n = strlen(projectId);
                const bool is_quickstart = (projectId_n > 11 /* len("quickstart-") */
                                            ...
                                            && projectId[10] == '-');
                if (!is_quickstart) return project_obj;
            }
        }
    }
    return NULL;
...
void no_projects_error() {
    fputs("A project must be created. For details on how and why, see: "
          "https://cloud.google.com/resource-manager/docs/creating-managing-projects", stderr);
    exit(EXIT_FAILURE);
}
