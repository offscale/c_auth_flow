...
inline bool is_sign(const char ch) { return ch == '+' || ch == '-'; }

inline bool is_exponent(const char ch) { return ch == 'e' || ch == 'E'; }

inline bool is_decimal_point(const char ch) { return ch == '.'; }

inline bool is_quote(const char ch) { return ch == '"' || ch == '\''; }

inline bool is_comma(const char ch) { return ch == ','; }

inline bool is_colon(const char ch) { return ch == ':'; }
...
inline bool is_digit(const char n) { return n >= '0' && n <= '9'; }
...
inline bool is_hex_digit(const char ch) {
  return ch == 'a' || ch == 'A' || ch == 'b' || ch == 'B' || ch == 'c' ||
         ch == 'C' || ch == 'd' || ch == 'D' || ch == 'e' || ch == 'E' ||
         ch == 'f' || ch == 'F' || is_digit(ch);
}
...
inline bool is_unreserved(const char ch) {
  return ch == '-' || ch == '.' || ch == '_' || ch == '~' || is_alpha(ch) ||
         is_digit(ch);
}

inline bool is_pchar(const char ch) {
  return ch == ':' || ch == '@' || is_unreserved(ch) || is_pct_encoded(ch) ||
         is_sub_delims(ch);
}
